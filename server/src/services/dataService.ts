...
import { Product, Category, ShopInfo, HomepageConfig, Review, StorePhoto } from '../types/index.js';
...
const initialStorePhotos: StorePhoto[] = [
  {
    id: "photo-1",
...
let memoryShopInfo: ShopInfo = { ...initialShopInfo };
let memoryHomepage: HomepageConfig = { ...initialHomepageConfig };
let memoryReviews: Review[] = [...initialReviews];
let memoryStorePhotos: StorePhoto[] = [...initialStorePhotos];
...
export const dataService = {
  ...
  async updateProduct(id: string, updates: Partial<Product>): Promise<Product> {
    ...
    if (firestore) {
      await firestore.collection('products').doc(id).update(updated as any);
    }
    const idx = memoryProducts.findIndex(p => p.id === id);
    if (idx !== -1) memoryProducts[idx] = updated;
    return updated;
  },
  ...
  async createCategory(data: Omit<Category, 'id'>): Promise<Category> {
    ...
    if (firestore) {
      await firestore.collection('categories').doc(newCat.id).set(newCat);
    }
  ...
  async updateCategory(id: string, updates: Partial<Category>): Promise<Category> {
    ...
    if (firestore) {
      await firestore.collection('categories').doc(id).update(updated as any);
    }
  ...
  async getShopInfo(): Promise<ShopInfo> {
    if (firestore) {
      const doc = await firestore.collection('shopInfo').doc('default').get();
      if (doc.exists) {
        return doc.data() as ShopInfo;
      }
    }
    return memoryShopInfo;
  },

  async updateShopInfo(updates: Partial<ShopInfo>): Promise<ShopInfo> {
    memoryShopInfo = { ...memoryShopInfo, ...updates };
    if (firestore) {
      await firestore.collection('shopInfo').doc('default').set(memoryShopInfo, { merge: true });
    }
    return memoryShopInfo;
  },
  ...
  async getHomepageConfig(): Promise<HomepageConfig> {
    if (firestore) {
      const doc = await firestore.collection('homepage').doc('config').get();
      if (doc.exists) {
  ...
  async updateHomepageConfig(updates: Partial<HomepageConfig>): Promise<HomepageConfig> {
    memoryHomepage = { ...memoryHomepage, ...updates };
    if (firestore) {
      await firestore.collection('homepage').doc('config').set(memoryHomepage, { merge: true });
    }
    return memoryHomepage;
  },

  // Reviews
  async getReviews(onlyVisible = false): Promise<Review[]> {
    let list = memoryReviews;
    if (firestore) {
      const snap = await firestore.collection('reviews').get();
      if (!snap.empty) {
        list = snap.docs.map(d => ({ id: d.id, ...d.data() } as Review));
      }
    }
  ...
  async createReview(data: Omit<Review, 'id'>): Promise<Review> {
    const rev: Review = { ...data, id: 'rev-' + Date.now() };
    if (firestore) {
  ...
  async updateReview(id: string, updates: Partial<Review>): Promise<Review> {
    ...
    if (firestore) {
      await firestore.collection('reviews').doc(id).update(updated as any);
    }
  ...
  // Store Photos
  async getStorePhotos(): Promise<StorePhoto[]> {
    if (firestore) {
      const snap = await firestore.collection('storePhotos').get();
      if (!snap.empty) {
        return snap.docs.map(d => ({ id: d.id, ...d.data() } as StorePhoto)).sort((a, b) => a.order - b.order);
      }
    }
    return memoryStorePhotos.sort((a, b) => a.order - b.order);
  },

  async addStorePhoto(data: Omit<StorePhoto, 'id'>): Promise<StorePhoto> {
    const photo: StorePhoto = { ...data, id: 'photo-' + Date.now() };
    if (firestore) {
      await firestore.collection('storePhotos').doc(photo.id).set(photo);
    }
    memoryStorePhotos = [...memoryStorePhotos, photo];
    return photo;
  },

  async deleteStorePhoto(id: string): Promise<void> {
    if (firestore) {
      await firestore.collection('storePhotos').doc(id).delete();
    }
